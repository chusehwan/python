...
def copy_right_value(file_name, tab_num, route):
    image_search_3tiems(file_name, route)
    # 탭
...
def image_search_3tiems(file_name, route):
    # ※주의※ 파일명 영어만 됨
    os.chdir(route)
    search_count = 0
    while True:
        location = pyautogui.locateCenterOnScreen(file_name)
        if location == None:
            print(file_name+'이미지 찾지 못함_count:'+str(search_count))
            search_count += 1
            if search_count > 5:
                pyautogui.press('tab')
                search_count = 0
...
for a in range(total_no):
    ...
    while True:
        ...
        if current_copied_employee_id != previous_copy_id and len_of_cpdid == 5:
            break
    # 저장된 사번 변수 엑셀파일에 저장
    save_value_in_excel(ws, current_copied_employee_id, a+init_cell_row, column_num = 1, route = origin_folder)
    pyautogui.PAUSE = 0.1
    # 파일 저장기능
